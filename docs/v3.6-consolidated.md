...
### F.4 `__all__` Static Analysis (Open)
...
`defmodule` extraction works but function capture `(call)` is too broad. Refine to `call . (do_block)` or `def`/`defp` patterns for more precise function extraction.

### F.6 Parameter/Return Type Annotation Filtering (v3.6)

`is_builtin_type()` is wired for class bases but not parameter annotations or return types because the walker doesn't extract those fields yet. Wire when annotation extraction is added to the walker.
