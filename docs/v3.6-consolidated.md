...
3. **Name-based resolution with import graph.** Primary resolution traces imports and scoping rules (18 languages). Falls back through signature matching, framework resolvers, and embedding similarity — each with disjoint confidence bands. (Stack Graphs deferred to post-v1.)
...
|------|-----------|------------|----------|
| Tier 1 | Python, TypeScript, JavaScript, Rust, Go, Java, C, C++, Ruby, PHP, C#, Kotlin (12) | Import → Signature → Framework | Full tool suite |
...
**v3.6 key changes:**
- Kotlin promoted from Tier 2 to Tier 1
- All 6 Tier 2 languages have grammar-verified `.scm` queries producing real entities (no fallback `(comment)` queries)
- 18 languages indexed with 16 `.scm` files (TypeScript + JavaScript share one)
...
│  │ • Semantic Resolution Engine (4-layer cascade):                │  │
│  │     L1: Import Graph + Scope (0.80-0.89)                       │  │
│  │     L2: Signature Matching (0.40-0.79)                         │  │
│  │     L3: Framework Resolvers (0.80-1.00)                        │  │
│  │   [L4 runs in Python: Embedding (0.20-0.39)]                   │  │
│  │   [Stack Graphs + LSP deferred to post-v1]                     │  │
...
/// Covers 70+ builtins: Rust i8-u128, C int/float, Scala Int/String,
/// Swift Int/Double, TS string/boolean, and more.
...
/// Matches 20+ literal node kinds across all supported grammars.
pub fn is_literal_receiver(kind: &str) -> bool;

/// Return true if name is a stoplisted identifier.
/// Covers 12 names: this, self, super, null, nil, true, false,
/// undefined, NULL, nullptr, None, new.
/// Applied at both extraction and resolution layers.
...
The projected graph is a **rebuildable derivative** of Macrame's ledger. It provides O(1) reverse-index lookups for structural Pest queries and L1/L2 resolution.
...
CodeRadar uses `tree-sitter-language-pack` v1.14 for all 18 languages, replacing 11 individual grammar crates. Languages are selected at compile time via `TSLP_LANGUAGES` in `.cargo/config.toml` with `TSLP_LINK_MODE=static`. Kotlin is now available through the pack (previously feature-gated behind a crate flag that didn't work reliably).
...
### 4.3 `.scm` Query Inventory

16 query files covering 18 languages (TypeScript + JavaScript share one). All query files use grammar-verified node kinds:

| Language | File | Key Capture Nodes |
|----------|------|-------------------|
...
**Query compilation is non-fatal.** If a grammar's node names don't match, a fallback chain `(comment) → (ERROR)` ensures the walker still produces a module entity rather than crashing. This keeps untested grammars functional at reduced fidelity.

**Swift disambiguation.** Swift uses a single `class_declaration` node kind for classes, structs, enums, protocols, and actors. `classify_class_like()` scans keyword children to determine the actual kind, storing the result with suffixes in `grammar_kind`: `class_declaration/class`, `class_declaration/struct`, `class_declaration/enum`, `class_declaration/interface`.

**Zig name extraction.** `FnProto` names come from the `function:` field; `VarDecl` names use `variable_type_function:` field checking for function-like types.

**R function name extraction.** Uses `binary_operator` parent check for function names on the left side of `<-` assignments.
...
**Strategy 1 — `preceding_docstring(node, source)`.** Collects the comment run immediately before a declaration, climbing wrapper nodes (`export_statement`, `decorated_definition`, `lexical_declaration`, `variable_declaration`, `ambient_declaration`). Strips comment markers (`#`, `//`, `/* */`, `/** */`, `--`, `--[[ ]]`, `(* *)`, `///`). Uses faithful `js_multiline_strip()` for `^/gm` replacement semantics. Based on CodeGraph's `preceding_docstring` (MIT License).

**Strategy 2 — `@docstring` capture fallback.** If the preceding scan finds nothing, falls back to the tagged inline docstring from `.scm` queries.

Applied to both `Tag::Class` and `Tag::Function` handlers in the walker.
...
| Pattern | Example | AST Context |
|---------|---------|-------------|
| Assignment RHS | `self.on_click = self.handle_click` | `assignment` → RHS identifier or dotted access |
| Return values | `return handler` | `return_statement` → identifier child |
| Keyword arguments | `on=handler` | `keyword_argument` → value identifier |
| Argument lists | `register_callback(handler)` | `argument_list` → identifier arguments |
| Dict/list values | `{"key": handler}`, `[handler1, handler2]` | `dictionary` pairs or `list` elements |

Assignment RHS supports dotted access: `attribute`, `field_expression`, `member_expression` — traversing the dot chain to resolve the full name.

**Post-walk resolution.** `resolve_fn_ref_candidates()` runs after the walk finishes, matching candidates against:

1. **Same-file function names** — all function entities extracted from the current file
2. **Imported names** (v3.6) — names from `FromImport`/`RelativeImport` entries in the same file, including aliases

This enables cross-file fn-ref resolution: `from .handlers import handle_click` followed by `self.on_click = handle_click` in another file correctly produces a `register → handle_click` edge.

**Deduplication.** No duplicate calls on the same function with the same (name, line, col).

### 4.8 Noise Filtering (v3.6)

**Builtin type filter — class bases.** `is_builtin_type(name)` covers 70+ builtins across all supported languages (Rust `i8`–`u128`, C `int`/`float`/`double`, Scala `Int`/`String`/`Boolean`, Swift `Int`/`Double`/`String`/`Bool`, TS `string`/`boolean`/`number`, etc.). Applied to class `bases` extraction to prevent `class Foo(int)` from generating a bogus `int` reference.

**Literal receiver filter.** `is_literal_receiver(kind)` matches 20+ literal node kinds (`string`, `integer`, `true`, `STRINGLITERALSINGLE`, `integer_literal`, `float_literal`, etc.). Applied to dotted call extraction to skip `"str".method()`, `5.times()`, and similar patterns.

**Name stoplist — dual-layer.** `is_stoplisted(name)` covers 12 names (`this`, `self`, `super`, `null`, `nil`, `true`, `false`, `undefined`, `NULL`, `nullptr`, `None`, `new`). Applied at:
- **Walker level** — all three call extraction paths (simple identifier calls, dotted calls, Zig `FnCallArguments`).
- **Resolution level** — `resolve_calls()` in the orchestrator produces `UnresolvedReason::Stoplisted` for any stoplisted target that slipped through extraction.
...
**MRO invalidation.** MRO for class `C` is invalidated when one of `C`'s direct bases changes, or any transitive ancestor of `C` changes its bases. Tracked via the `subclasses` reverse index. Bounded at `max_mro_invalidation_depth` (default 50).

### 5.4 Incremental Update Semantics (Macrame)

1. **Retire old entities/edges** — `db.retire_entity(id, valid_to=now)` sets `valid_to`, doesn't delete.
2. **Assert new entities/edges** — `db.upsert_concept(...)` / `db.assert_edge(...)` with `valid_from=now`.
...
### 6.1 Four-Layer Resolution Cascade

```
LAYER 1  Import Graph + Scope    confidence 0.80 – 0.89
LAYER 2  Signature Matching      confidence 0.40 – 0.79
LAYER 3  Framework Resolvers     confidence 0.80 – 1.00
...
> **v0.5 note:** Stack Graphs (formerly L1, 0.90–1.00) and LSP Override (formerly L5, 1.00) deferred to post-v1. CodeGraph ships 30+ languages at production scale with zero Stack Graphs or LSP dependency — proving compiler-grade disambiguation is not required for the MCP agent use case.
...
### 6.2 Layer 1 — Import Graph + Scope

Traces import statements (`import`, `from X import Y`, `require`, `use`, `mod`) to resolve cross-file references. Uses per-language import path heuristics to map qualified names to file paths. Confidence clamped to [0.80, 0.89].

### 6.3 Layer 2 — Signature Matching (was §6.3)
...
| Both have `__all__` | **Union.** Names in both → one entry, `file_type = Impl`. Names only in stub → `Stub`. Names only in impl → `Impl`. | **Impl's `__all__` only** (Python runtime semantics) |
...
**Annotation provenance rule.** Annotations in stub files are *not* backfilled into impl entities — an impl `Constant`/`Field`/`Parameter` whose annotation is only present in the stub retains a `None` annotation in the graph (lossless: the stub entity keeps it).
...
`resolve_calls()` in the orchestrator checks `is_stoplisted()` on unresolved call targets. Matches produce `Unresolved { reason: Stoplisted }` — catching any stoplisted names that slipped past the walker-level filter. This is a defense-in-depth measure: the walker filters at extraction time, and the orchestrator catches anything remaining.

### 6.8 Staged Two-Phase Commit
...
### 7.3 Rust-Accelerated Traversal

`call_chain` and `impact_analysis` intents are served from the in-memory `CallGraph` (O(1) neighbor access via `StableDiGraph`).

### 7.4 MCP Search (v3.6)

`search_entities(query, top_k, kind)` supports optional `kind` parameter to filter by `function`/`class`/`module`. This enables agents to scope searches: "find all classes matching `User`" vs "find all functions matching `login`".
...
# Agent exploration (Macrame — includes framework edges v3.6)
...
### 8.5 v3.6: `register_synthetic_edge`
...
After registration, `callers_of("views.py::UserView")` returns the route node, and `callees_of("django:route:users/")` returns the handler. Framework resolvers call this automatically during `analyze`/`init` — no manual invocation needed.
...
The §22.1 <100 ms structural query target is only achievable with in-memory reverse indexes. Macrame handles everything else.
...
| `decorators` | string | NUL-joined list |
| `content_hash` | string | `"a1b2c3d4e5f6"` |
| `parse_quality` | string | `"clean"` |
| `return_type` | string | `"User"` |
| `grammar_kind` | string | `"class_declaration/struct"` (v3.6) |
...
| `HANDLES` | route | function, class | v3.6: framework-synthesized (Django path→view) |
| `REGISTERS` | router | viewset, handler | v3.6: DRF router.register, Flask add_resource |
...
### 10.5 v3.6 Persistence Details

**Timestamp format.** Macrame requires canonical ISO 8601: `YYYY-MM-DDTHH:MM:SS.ffffffZ`. Empty strings are rejected during normalization. Callers must compute explicit timestamps before upsert (using `civil_from_days()` algorithm).

**FK constraint handling.** `write_bulk_atomic` enforces that source and target entities exist in the `concepts` table. Edges targeting `external::` or `builtins.` prefixes have no concept entry and must be filtered before assertion. `persist_edges()` applies this filter automatically.

**Batch size.** Edges are persisted in batches of 200 to amortize write overhead.

**Edge registration.** `register_synthetic_edge()` persists a single `EdgeAssertion` for each framework-produced edge, using the edge kind string (e.g., `HANDLES`, `REGISTERS`) as the edge type.
...
Via the `ignore` crate: `.gitignore` + `.harnessignore` + built-in defaults. `.coderadar/` is in `.gitignore` to prevent test artifacts from being staged.
...
deny  = [".git/", ".harness/", ".coderadar/", "/migrations/", "/*.lock", "/generated/"]
...
### 15.1 Build Configuration (v3.6)

```toml
# .cargo/config.toml — tree-sitter-language-pack static compilation
[env]
TSLP_LANGUAGES = "python,typescript,javascript,go,rust,java,c,cpp,ruby,php,csharp,kotlin,swift,scala,lua,elixir,zig,r"
...
```
coderadar init <path>                  Initial analysis (includes framework extraction)
coderadar analyze <path>               One-shot analysis
coderadar update <file> [--content -]  One-shot update
coderadar watch <path> [--debounce MS] Live file watcher (v3.6)
...
coderadar callers <qualified-name>     List callers (includes framework edges v3.6)
...
`analyze` and `init` automatically run framework resolvers after tree-sitter extraction:
1. Each resolver's `detect()` checks project structure (e.g., `manage.py` for Django)
2. Matching resolvers `extract()` route nodes and handler edges from Python files
3. Synthetic edges are registered in the Rust graph via `register_synthetic_edge()`
4. Summary output shows `routes`, `handlers`, and `edges_registered` counts

Framework edges are immediately queryable via `callers_of()` / `callees_of()` / `explore()`.
...
**Filename-based filtering.** Extensions are checked against supported language extensions. Noise paths (`.git/`, `node_modules/`, `__pycache__/`, `*.pyc`, `*.log`, etc.) are excluded before parsing.

**Same-path deduplication.** Within each batch, same-path events are collapsed to the latest event.
...
**WriteGuard:** Every mutation path registers with `WriteGuard`; watcher's `should_drop()` check suppresses self-write events.

**Python bindings:** `start_watcher(paths, debounce_ms)`, `next_watcher_batch()`, `stop_watcher()` via a `Mutex<Option<FileWatcher>>` global. CLI `coderadar watch [PATHS...] --debounce MS`.
...
**Graphviz v3.6 wiring:**
- `_dot_dependency_graph()` extracts real module→module edges via `_extract_module_edges()`
- `_dot_class_hierarchy()` walks real class entities with `parent_class` + `callees_of` for inheritance edges
- Kosaraju SCC detection uses iterative DFS (no stack overflow on deep graphs)
- `_find_sccs()` returns strongly-connected components for cycle highlighting
- New helpers: `_safe_id()`, `_short_name()`, `_iter_modules()`, `_iter_classes()`
- CLI `--format graphviz` passes live `CodeGraph` to `generate_dot()`
...
| Tier 1 | Python, TypeScript, JavaScript, Rust, Go, Java, C, C++, Ruby, PHP, C#, Kotlin | 12 | Import → Signature → Framework | Full tool suite |
| Tier 2 | Swift, Scala, Lua, Elixir, Zig, R | 6 | Import → Signature | `replace_body`, `create_entity` |
| Tier 3 | Erlang, Haskell, OCaml, Nim, Dart, Julia, Perl, Shell, SQL, HTML, CSS, YAML, TOML, JSON, Markdown + 341 more | 347 | Signature Match only | `replace_body`, `create_entity` |

### 20.2 Tier 2 Grammar Notes
...
| **Zig** | `FnProto`, `VarDecl`, `FnCallArguments` | Name extraction via `function:` and `variable_type_function:` fields. Calls via `FnCallArguments` → parent `SuffixExpr` |
| **R** | `function_definition`, `call`, `comment` | Function name via `binary_operator` parent check for `<-` assignments |
| **Scala** | `class_definition`, `object_definition`, `function_definition` | Full extraction via standard node kinds; children walk for name extraction |
| **Lua** | `function_declaration`, `function_call`, `local_variable_declaration` | Full extraction |
| **Elixir** | `defmodule`, `call` | Module extraction via `defmodule`. Function calls via `(call)` capture (refinement to `def`/`defp` patterns deferred) |

### 20.3 Adding a New Language

1. Add language variant to `Language` enum and `pack_name()` mapping
2. Add to `TSLP_LANGUAGES` in `.cargo/config.toml`
3. Write `queries/<lang>.scm` with grammar-verified capture node kinds
4. Implement walker name extraction and call handling branches
5. Write `.tsg` rule file (Tier 1) or configure import patterns (Tier 2)
6. Add fixture files and golden extraction tests
7. Extend `is_builtin_type()` with language-specific builtins
...
### 23.1 Test Pyramid & Current Coverage (v3.6)

Unit 70% / Integration 25% / E2E 5%.

**Current: 174 tests passing.**

| Suite | Count | Coverage |
|-------|-------|----------|
| Rust (`core_indexer`) | 152 | Graph, walker, tagger, resolution, query, mutation, storage, delta, spans, diff/patch, persistence, watcher, grammar_kind, fn-ref, synthetic edges |
| Python framework resolvers | 22 | Django, Flask, FastAPI, DRF, Flask-RESTful, `.as_view` stripping, detection, extraction, edge registration |
| MCP server | Integrated into Rust suite | Server creation, tool guards, kind-filtered search |
...
- `grammar_kind` tests (Python, TypeScript, Rust, Swift class + struct, Zig)
- Function-as-value reference capture (5 patterns: callback assignment, return, kwargs, args, dict)
- Synthetic edge registration (2 tests)
- Cross-file fn-ref via imports
- Watcher test suite (8 tests)
...
- **Removed from v3.6:** 11 individual tree-sitter grammar crates (replaced by `tree-sitter-language-pack`)
...
| LSP pool integration | Deferred to post-v1 |
| Stack Graphs `.tsg` rules | Deferred to post-v1 |
...
| **Flask** | `@app.route` patterns | route registration → route nodes + handler edges | Flask-RESTful `add_resource()` extraction (multi-arg paths) |
| **FastAPI** | `APIRouter` imports | `@app.get()` / `@router.post()` → route nodes | `Depends()` dependency injection chain extraction |
...
This is called automatically by the CLI during `analyze` and `init`. Results include `edges_registered` count in the framework extraction summary.

### 28.4 AST Call Handling (v3.6)

All three resolver `_get_name()` methods now handle `ast.Call` nodes (traverse into `.func`), enabling extraction of handler names from expressions like `views.MyView.as_view()`, `Depends(get_db)`, and `router.register(...)`. The `.as_view()` suffix is stripped in Django's `_get_route_handler()`.

### 28.5 Synthetic Edge Provenance
...
| `grammar_kind` | String | O(1) | Extracted metadata (v3.6) |
...
| **v3.6 Synthetic edge integration** | `register_synthetic_edge()` bridge (§8.5, §16.1, §28.3) |

Key resolutions carried forward from v3.3:
...
core_indexer/src/
  ...
  extract/
    ...
    walker.rs              # Pass 2 hierarchy walker
                           ...
                           #         import-aware fn-ref resolution, stoplist filtering
...
py_agent/src/coderadar/
  ...
  mcp/
    ...
    explore.py, search.py, node.py, affected.py, budget.py
  visualizers/
    mermaid.py
...
|------|------|--------|------|
| 0.80–0.89 | L1 | ImportConstrained | import can be traced to target file |
| 0.40–0.79 | L2 | SignatureMatch | name+sig match across files |
| 0.80–1.00 | L3 | FrameworkResolver | framework pattern matches (Django, Flask, etc.) |
| 0.20–0.39 | L4 | Embedding (Python) | cosine ≥ 0.85 |
| unset | — | Unresolved | no layer matched; reason recorded (includes Stoplisted v3.6) |

> **v0.5:** Stack Graphs (formerly L1, 0.90–1.00) and LSP Override (formerly L5, 1.00) deferred to post-v1.

---

## Appendix F: Open-Question Decisions

### F.1 Wildcard Import Multi-Hop (Resolved)
...
### F.4 `__all__` Static Analysis — ✅ Partially resolved (v0.4.1)

Detection module (`py_agent/src/coderadar/resolvers/exports.py`) extracts `__all__ = [...]`, `+=`, `.extend()`, `.append()` patterns with 7 tests. Pipeline integration (`star_exports` plumbing in Rust `import_graph.rs`) deferred — detection results are available but not yet wired into wildcard-import resolution.

### F.5 Elixir Function Query Refinement — ✅ Resolved (v0.4.1)

`elixir.scm` rewritten with `(call target: (identifier) @_def_target (do_block) @_body) @function.def` and `#match? @_def_target "^(def|defp)$"` predicate. Module capture uses `defmodule` target. Name extraction via `arguments → alias` for modules and `arguments → call → target` for functions.

### F.6 Parameter/Return Type Annotation Filtering — ✅ Resolved (v0.4.1)

`extract_parameters()` handles `typed_parameter`, `default_parameter`, `typed_default_parameter`, and `keyword_argument` nodes. `is_builtin_type()` applied to both annotation and return type fields. 2 Rust tests.

### F.7 LSP Pool Integration — Deferred to post-v1 (v0.4.1 decision)

CodeGraph ships 28 framework resolvers plus name-matching against an indexed symbol table without any LSP dependency — proving LSP is not required for production-quality resolution. The LSP pool (`py_agent/src/coderadar/lsp/pool.py`, 204 lines of stubs) would add JSON-RPC process management, per-language server spawning (pyright, rust-analyzer, gopls), and stdio transport for a 5th resolution layer at confidence 1.00. Deferred because: (1) CodeGraph's approach validates the AST-only path, (2) the plumbing is non-trivial (JSON-RPC framing, thread safety, process lifecycle), and (3) expanding framework resolvers and Stack Graphs coverage delivers more resolution improvement per hour.

### F.8 Framework Resolver Expansion + Query-Time Resolution — Planned for v0.6–v0.9 (pre-v1.0)

CodeRadar has 3 Python-only framework resolvers (Django, Flask, FastAPI) vs CodeGraph's 25 resolvers across 12 languages. Plan to close this gap in two phases:

**Phase 1 — Add 6 backend framework resolvers (v0.6–v0.7, ~6h):**

| Resolver | Language | Key patterns | Lines (est.) |
|----------|----------|-------------|-------------|
| Go (Gin/Echo/Chi/Fiber) | Go (Tier 1) | `r.GET("/path", handler)`, middleware chains, handler groups | 150 |
| Rust (Actix/Axum/Rocket) | Rust (Tier 1) | `.route("/", web::get().to(h))`, `#[handler]`, extractors | 250 |
| Java/Spring | Java (Tier 1) | `@GetMapping`, `@Autowired`, JPA repository conventions | 300 |
| Ruby/Rails | Ruby (Tier 1) | ActiveRecord associations, `before_action`, `has_many :through` | 200 |
| Laravel | PHP (Tier 1) | Eloquent ORM, service container, `Route::get()` | 200 |
| ASP.NET | C# (Tier 1) | `[HttpGet]`, `[FromServices]`, `IActionResult`, DI | 150 |

Each follows the existing `detect → extract_routes → register_synthetic_edge` pattern from Django/Flask/FastAPI.

**Phase 2 — Query-time resolution (v0.8, ~3h):**

Currently all resolution happens at index time via `SyntheticEdge`. CodeGraph's key advantage is `resolve(ref, context)` — framework resolvers that run at **query time** to bridge patterns invisible at index time:
- Django ORM dynamic dispatch: `self._iterable_class()` → `ModelIterable.__iter__()`
- Spring service container: `@Autowired MyService service` → singleton bean
- Go handler conventions: any function named `Handle*` in a `handlers/` dir → potential gin/echo handler

Requires: add `resolve()` method to `FrameworkResolver` base class, call from `_resolve_names()` / `_explore()` in MCP server, pass resolution context (file list, known symbols) to each resolver.

**Phase 3 — Remaining languages (v0.9, ~4h):**

Add framework resolvers for Tier 2 languages once they have full Stack Graphs support (see F.9). Candidates: Swift (SwiftUI/Vapor), Kotlin (Spring Boot), Scala (Play), Elixir (Phoenix), Lua (Lapis). JS ecosystem resolvers (React, Vue, Express, NestJS, etc.) remain deferred — they require JSX-level resolution that CodeRadar doesn't target.

**Design constraint:** All lifted from CodeGraph's patterns (`src/resolution/frameworks/*.ts`, MIT License) — attributed in docstrings. No copy-paste of complete modules; adapt the logic to Python + CodeRadar's resolver base class.
...
| Technique | Source File | CodeRadar Location |
|-----------|------------|-------------------|
| `preceding_docstring()` | `codegraph-kernel/src/docstring.rs` | `core_indexer/src/extract/docstring.rs` |
| `clean_comment_markers()` | `codegraph-kernel/src/docstring.rs` | `core_indexer/src/extract/docstring.rs` |
| `is_builtin_type()` | `codegraph-kernel/src/swift.rs`, `python.rs` | `core_indexer/src/types.rs` |
| `is_literal_receiver()` | function-ref patterns across walkers | `core_indexer/src/types.rs` |
| `is_stoplisted()` | `codegraph-kernel/src/python.rs`, `swift.rs`, `tsjs/fnref.rs` | `core_indexer/src/types.rs` |
| `classify_class_like()` | `codegraph-kernel/src/swift.rs`, `kotlin.rs` | `core_indexer/src/extract/walker.rs` |
| `scan_for_fn_ref()` / `maybe_capture_fn_refs()` | `codegraph-kernel/src/python.rs`, `swift.rs` | `core_indexer/src/extract/walker.rs` |
| `resolve_fn_ref_candidates()` | fn-ref flush pattern | `core_indexer/src/extract/walker.rs` |
| Framework resolver patterns | `src/resolution/frameworks/python.ts` | `py_agent/src/coderadar/resolvers/` |
| MCP tool surface design | `src/mcp/tools.ts`, `server-instructions.ts` | `py_agent/src/coderadar/mcp/` |
| Agent interface principles | Production deployment observations | §26 Agent Interface Design |

Each technique carries the following attribution block:
```rust
...
- Static compilation via `TSLP_LANGUAGES` + `TSLP_LINK_MODE=static` in `.cargo/config.toml`
- Removed `kotlin` feature gate; Kotlin now works (promoted to Tier 1)
- `Language::pack_name()` maps enum variants → language-pack names
- 120 tests

### v0.3.10 — Live File Watcher
- `FileWatcher` rewritten from stub with `notify` + `notify-debouncer-mini` + `crossbeam-channel`
- `EventBridge` implements `DebounceEventHandler`
- Filename filter excludes non-source extensions and noise paths
- Same-path deduplication within batches
- Python bindings: `start_watcher()`, `next_watcher_batch()`, `stop_watcher()` via `Mutex<Option<FileWatcher>>`
- CLI `coderadar watch [PATHS...] --debounce MS`
- 123 tests

### v0.3.11 — Graphviz Visualizer
- Complete rewrite with real graph data
- `_dot_dependency_graph()` extracts module→module edges; `_dot_class_hierarchy()` walks real classes
- Kosaraju SCC converted to iterative DFS (no stack overflow)
- `_find_sccs()` returns strongly-connected components
- New helpers: `_safe_id()`, `_short_name()`, `_iter_modules()`, `_iter_classes()`
- CLI `--format graphviz` passes live `CodeGraph`
- 123 tests

### v0.3.12 — End-to-End Integration Tests
- `tests/test_e2e.py` (339 lines, 21 tests)
- Fixture project `e2e_project/` with `models.py` + `services.py`
- `TestFixtureFiles` (4 tests), `TestFullPipeline` (13), `TestVisualizerPipeline` (5), `TestMutationPipeline` (1), `TestEmbeddingPipeline` (2)
- Rust-dependent tests skip via `@pytest.mark.skipif`
- 123 Rust + 21 Python = 144 tests

### v0.3.13 — Macrame Write-Through
- `persist_edges()` flushes `callees_by_caller` edges as `EdgeAssertion(CALLS)` in batches of 200
- Called after `resolve_all_calls()` in both `analyze()` and `update_file()` paths
- Fixed store path: `.coderadar/store/coderadar.db` (was directory)
- 4 persistence tests; 127 Rust tests

### v0.3.14 — MCP Server Validation
- 4 tool guards changed from `if graph is None` to `graph_stats()` real check
- `search_entities(query, top_k, kind)` — optional kind parameter filters by function/class/module
- 20 MCP tests; `.gitignore` updated with `.coderadar/`
- 147 tests total

### v0.3.15 — 6 Tier 2 Languages
- Swift, Scala, Lua, Elixir, Zig, R added
- 6 new `.scm` files, tagger/walker extensions, watcher extension
- Scala/Lua/Elixir full indexing; Swift/Zig/R with initial (pre-grammar-fix) queries
- 10 new tests; 136 Rust tests

### v0.3.16 — CodeGraph-Informed v3.6 Improvements
- **`.scm` query fixes:** Swift/Zig/R rewritten with grammar-verified node kinds
- **`grammar_kind`:** Universal field on Class; `classify_class_like()` for Swift disambiguation
- **Builtin type filter:** `is_builtin_type()` (70+ types), `is_literal_receiver()` (20+ node kinds)
- **Name stoplist:** `is_stoplisted()` (12 names) at extraction + resolution (`UnresolvedReason::Stoplisted`)
- **Docstring extraction:** `preceding_docstring()` wired for all class/function handlers
- **Fn-ref capture:** `scan_for_fn_ref()` 5 patterns, `resolve_fn_ref_candidates()` with import-aware matching
- **Framework resolvers:** DRF `router.register()`, Flask `add_resource()`, `.as_view()` stripping
- **`ast.Call` handling:** All three resolver `_get_name()` methods handle call nodes
- Kotlin `call_expression` uses `callee` field for name extraction
- `interface_declaration` added to TypeScript `.scm` query
- 149 Rust + 21 Python = 170 tests

### v0.4.0 — v3.6 Completion
- **Synthetic edge bridge:** `register_synthetic_edge()` on CodeGraph; PyO3 wrapper; CLI wiring
- **Framework edge registration:** `_run_framework_extraction()` calls `register_synthetic_edge()` for every resolver-produced edge
- **Cross-file fn-ref:** `resolve_fn_ref_candidates()` uses imported names from `FromImport`/`RelativeImport`
- **Parallel `_get_name()` fix:** Django, Flask, and FastAPI resolvers handle `ast.Call`
- 152 Rust + 22 Python = 174 tests
