// Per-language indexing pipeline tests — moved verbatim from graph/tests/mod.rs (step 15).
...
fn test_kotlin_class_indexing() {
    ...
    index_source(
        &graph,
        "class Person(val name: String) { fun greet() {} }\n",
        "Person.kt",
    );
    let snap = graph.snapshot();
    assert!(
        snap.classes.values().any(|c| c.name == "Person"),
...
fn test_kotlin_call_indexing() {
    ...
    let snap = graph.snapshot();
    assert!(
        snap.functions.values().any(|f| f.name == "foo"),
...
fn test_typescript_function_indexing() {
    ...
    assert!(
        snap.functions.values().any(|f| f.name == "hello"),
...
fn test_typescript_class_indexing() {
    ...
    let snap = graph.snapshot();
    assert!(
        snap.classes.values().any(|c| c.name == "Animal"),
        "Should have class Animal"
    );
    assert!(
        snap.functions.values().any(|f| f.name == "speak"),
...
",
    ...
    assert!(
        sub.bases.iter().any(|b| b.name == "X.Y"),
    ...
    assert!(
        i.bases.iter().any(|b| b.name == "G.H") && i.bases.iter().any(|b| b.name == "J"),
...
fn test_go_struct_indexing() {
    ...
    let snap = graph.snapshot();
    assert!(
        snap.classes.values().any(|c| c.name == "Dog"),
        "Should have Go struct Dog"
    );
    assert!(
        snap.functions.values().any(|f| f.name == "Bark"),
...
fn test_java_class_indexing() {
    ...
    let snap = graph.snapshot();
    assert!(
        snap.classes.values().any(|c| c.name == "Cat"),
        "Should have Java class Cat"
    );
    assert!(
        snap.functions.values().any(|f| f.name == "meow"),
...
fn test_cpp_function_indexing() {
    ...
    let snap = graph.snapshot();
    assert!(
        snap.functions.values().any(|f| f.name == "add"),
...
fn test_cpp_class_indexing() {
    ...
    let snap = graph.snapshot();
    assert!(
        snap.classes.values().any(|c| c.name == "Widget"),
        "Should have C++ class Widget"
    );
    assert!(
        snap.functions.values().any(|f| f.name == "render"),
...
fn test_ruby_module_indexing() {
    ...
    let snap = graph.snapshot();
    assert!(
        snap.classes.values().any(|c| c.name == "Utilities"),
...
fn test_php_class_indexing() {
    ...
    let snap = graph.snapshot();
    assert!(
        snap.classes.values().any(|c| c.name == "User"),
...
fn test_csharp_class_indexing() {
    ...
    let snap = graph.snapshot();
    assert!(
        snap.classes.values().any(|c| c.name == "Service"),
...
fn test_csharp_call_indexing() {
    ...
    let snap = graph.snapshot();
    assert!(
        snap.functions.values().any(|f| f.name == "A"),
...
fn test_cross_file_resolution_same_dir() {
    ...
    assert!(
        !callees.is_empty(),
        "process should have at least one callee, got {:?}",
        callees
    );
    assert!(
        callees.contains(&helper_id.to_string()),
        "process should call {}, got {:?}",
        helper_id,
        callees
    );
    ...
    assert!(
        callers.contains(&process_id.to_string()),
        "helper should be called by process, got {:?}",
        callers
    );
}
#[test]
fn test_cross_file_resolution_nested_package() {
    ...
    assert!(
        !callees.is_empty(),
        "make_cfg should have at least one callee, got {:?}",
        callees
    );
}
...
fn test_rust_method_resolution() {
    ...
    assert!(
        callees.contains(&baz_id.to_string()),
        "bar() should call baz() via self, got {:?}",
        callees
    );
}
#[test]
fn test_class_methods_populated_27() {
    ...
    assert!(
        ...
        "class.methods should list bar and baz, got {:?}",
        names
    );
    // Deterministic ordering (sorted by EntityId).
    assert!(
        foo.methods.windows(2).all(|w| w[0] <= w[1]),
...
fn test_scala_class_indexing() {
    ...
    index_source(
        &graph,
        "class User(name: String) { def greet(): Unit = {} }\ntrait Service { def run(): Unit }\n",
        "user.scala",
    );
    let snap = graph.snapshot();
    assert!(
...
fn test_zig_struct_indexing() {
    ...
    let snap = graph.snapshot();
    assert!(
        snap.classes.values().any(|c| c.name == "Point"),
...
fn test_fn_ref_assignment_callback() {
    ...
    let snap = graph.snapshot();
    assert!(
        snap.functions.values().any(|f| f.name == "handle_click"),
...
fn test_fn_ref_return_value() {
    ...
    let snap = graph.snapshot();
    let get_handler = snap.functions.values().find(|f| f.name == "get_handler");
...
fn test_fn_ref_no_false_positives() {
    ...
    let snap = graph.snapshot();
    let foo = snap.functions.values().find(|f| f.name == "foo");
...
fn test_fn_ref_argument_list() {
    ...
    let snap = graph.snapshot();
    let setup = snap.functions.values().find(|f| f.name == "setup");
...
fn test_fn_ref_dict_values() {
    ...
    let snap = graph.snapshot();
    let make_reg = snap.functions.values().find(|f| f.name == "make_registry");
...
fn test_grammar_kind_python_class() {
    ...
    assert_eq!(
        cls.grammar_kind, "class_definition",
...
fn test_grammar_kind_typescript_class() {
    ...
    let cls = snap
        ...
        .unwrap();
    assert_eq!(
        cls.grammar_kind, "class_declaration",
...
fn test_grammar_kind_swift_struct() {
    ...
    assert_eq!(
        cls.grammar_kind, "class_declaration/struct",
...
fn test_parameter_annotations_extracted() {
    ...
    assert!(
        func.parameters[2].annotation.is_some(),
        "Optional[str] is not a bare builtin"
    );
...
fn test_return_type_builtin_filtered() {
    ...
    assert!(
        func.unwrap().return_type.is_none(),
        "int return type should be filtered"
    );
}
