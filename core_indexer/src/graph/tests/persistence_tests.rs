...
fn test_synthetic_edge_registration() {
    ...
    graph
        .register_synthetic_edge("django:route:users/", "views.py::user_detail", "HANDLES")
        .unwrap();
    ...
    assert!(
        callees.unwrap().contains("views.py::user_detail"),
    ...
    assert!(
        callers.unwrap().contains("django:route:users/"),
...
fn test_temporal_edge_persistence_across_indexes() {
    ...
    assert!(
        meta.len() > 0,
        "db should have persisted data; size={}",
        meta.len()
    );

    // Second index should not corrupt
    index_source(&graph, "def x(): pass\n", "extra.py");
...
fn test_persist_edges_emits_imports_and_extends() {
    ...
    assert!(
        importer_edges > 0,
        "fixture should resolve >=1 import, got {}",
        importer_edges
    );
    assert!(
        subclass_edges > 0,
        "fixture should resolve >=1 subclass, got {}",
        subclass_edges
    );
    assert!(
        override_edges > 0,
        "fixture should resolve >=1 override, got {}",
        override_edges
    );

    let persisted = graph
        .persist_edges(&projection)
...
fn test_temporal_synthetic_edge_persistence() {
    let (graph, _dir) = graph_with_temp_store();
    index_source(&graph, "def user_detail(id): pass\n", "views.py");
...
fn test_persist_edges_scoped_writes_fewer_edges_than_unscoped() {
    ...
    assert!(
        scoped < all,
        "scoped {} should be below unscoped {}",
        scoped,
        all
    );
}
...
fn probe_synthetic_persist_error() {
    ...
    let res = store.assert_edges_bulk(vec![macrame::graph::EdgeAssertion::new(
        "main.py::run",
        "helpers.py::combine",
        "CALLBACK",
    )
    ...
    let res2 = store.assert_edges_bulk(vec![macrame::graph::EdgeAssertion::new(
        "helpers.py::combine",
        "main.py::run",
        "DEPENDS_ON",
    )
